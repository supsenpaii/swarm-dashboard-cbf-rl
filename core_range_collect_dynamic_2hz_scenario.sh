#!/usr/bin/env bash
# Variant of core_range_collect_dynamic_scenario.sh for the
# CORE_RANGE_2HZ_DYNAMIC_RECOLLECTION_AND_RETRAIN task.
#
# Only two things differ from the unmodified original:
#   1. --dataset-role core_range_dynamic_2hz_development (distinct dataset
#      role for this new corpus, so it can never be mistaken for or mixed
#      with the original 7.5Hz-era core_range_dynamic_development corpus).
#   2. --capture-timeout-s raised from the original's fixed 30s to 90s.
#      core_range_dynamic_capture.py's capture() requires 5 raw_range_computed
#      rows *before* motion starts within this timeout; at the new 2.0Hz
#      depth-submission default that is at most 60 submission attempts in
#      30s vs up to 180 in 90s, and calibration/RANSAC convergence variance
#      observed during the depth-scheduler-contention-fix sweep (some 2-5Hz
#      live sessions needed materially more wall-clock time than others to
#      reach a stable calibration+RANSAC fit) makes the original 30s floor
#      too tight to rely on. 90s matches the value already used and proven
#      sufficient for fresh capture attempts during that prior task.
# Stack launch, static environment setup, and the capture invocation itself
# are otherwise identical to the original script.

set -Eeuo pipefail

if [[ $# -ne 16 ]]; then
  echo "usage: $0 ID START END LATERAL Z YAW_START YAW_END DURATION HOLD RATE PITCH BBOX_X BBOX_Y BBOX_W BBOX_H OUTPUT" >&2
  exit 2
fi

scenario_id="$1"; start_range="$2"; end_range="$3"; lateral="$4"; target_z="$5"
yaw_start="$6"; yaw_end="$7"; duration="$8"; hold="$9"; rate="${10}"; pitch="${11}"
bbox_x="${12}"; bbox_y="${13}"; bbox_w="${14}"; bbox_h="${15}"; output_root="${16}"
...
env \
  ...
  SWARM_RANGE_DATASET_RUN_ID="core_dynamic_2hz_${scenario_id}" \
...
env \
  ...
  --dataset-role core_range_dynamic_2hz_development \
