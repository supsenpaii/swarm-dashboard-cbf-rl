#!/usr/bin/env bash
# Near-copy of core_range_collect_dynamic_scenario.sh (unmodified elsewhere
# in this repo) with two knobs exposed: --minimum-raw-frames and
# --capture-timeout-s. Needed because live-stack inference is exactly the
# slow thing this task is investigating, so the original script's fixed
# `--minimum-raw-frames 40 --capture-timeout-s 30` can legitimately time
# out before 40 raw_range_computed frames accumulate. Everything else
# (stack launch, static environment setup, capture invocation) is
# identical to the original script.

set -Eeuo pipefail

if [[ $# -ne 18 ]]; then
  echo "usage: $0 MIN_RAW_FRAMES CAPTURE_TIMEOUT_S ID START END LATERAL Z YAW_START YAW_END DURATION HOLD RATE PITCH BBOX_X BBOX_Y BBOX_W BBOX_H OUTPUT" >&2
  exit 2
fi

min_raw_frames="$1"; capture_timeout_s="$2"; scenario_id="$3"; start_range="$4"; end_range="$5"
lateral="$6"; target_z="$7"; yaw_start="$8"; yaw_end="$9"; duration="${10}"; hold="${11}"
rate="${12}"; pitch="${13}"; bbox_x="${14}"; bbox_y="${15}"; bbox_w="${16}"; bbox_h="${17}"
output_root="${18}"
...
env \
  ...
  SWARM_RANGE_DATASET_RUN_ID="core_contention_${scenario_id}" \
...
env \
  ...
  --minimum-raw-frames "${min_raw_frames}" --capture-timeout-s "${capture_timeout_s}" \
